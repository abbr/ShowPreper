...
export default class extends Draggable.draggableMixin(
  ...
  getGradientItemIndexFromAttrs(parsedGradientObject, attrs) {
    return (
      ...
      parsedGradientObject.colorStops.findIndex(x => {
  ...
  updateMarkerPosition(attrs, pct) {
    ...
    if (attrs) {
      ...
      if (colorStops.length > 0) {
        ...
        let dist =
          colorStops[rightMarkerIdx].p - colorStops[leftMarkerIdx].p || 1
        ...
        for (let i = 0; i < 4; i++) {
          ...
          newColor[i] = Math.max(
            ...
            Math.min(
              i < 3 ? 255 : 1,
              i < 3 ? Math.round(decimalVal) : decimalVal.toFixed(2)
            )
          )
        }
    ...
    try {
      ...
      gradientMarkers =
        ...
        colorStops.map((e, i) => {
          let pressed = false
          if (
            this.state.pressedMarkerAttrs &&
            this.state.pressedMarkerAttrs.c === e.c &&
            Math.abs(this.state.pressedMarkerAttrs.p - e.p) < 0.001
          ) {
            pressed = true
          }
          return (
    ...
    return (
      <div className="sp-gradient-panel-container">
        <div className="sp-gradient-panel-base">
          <div
            className="sp-gradient-panel"
            ref="gradientPanel"
            style={{
              background: 'linear-gradient(to right, ' + gradientArrString + ')'
            }}
          />
        </div>
        <div
          className="sp-gradient-marker-panel"
          title="Insert color stop here"
